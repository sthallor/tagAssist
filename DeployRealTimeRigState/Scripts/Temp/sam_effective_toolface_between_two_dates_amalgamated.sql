CREATE DEFINER=`root`@`localhost` PROCEDURE `sam_effective_toolface_between_two_dates_amalgamated`(IN `start_tstamp` BIGINT, IN `stop_tstamp` BIGINT)
	LANGUAGE SQL
	NOT DETERMINISTIC
	CONTAINS SQL
...
BEGIN
   ...
   select concat('create temporary table temp_sqlth_data_1 select t_stamp,tagid,floatvalue,intvalue from ',Partition1,' where t_stamp>=',start_tstamp-300000,' and t_stamp<=',stop_tstamp,' and tagid in (',HoleDepthTagID,',',ToolfaceTagID,',',QuillOscEnabledTagID,',',BitDepthTagID,')') into @s;
   ...
   if (isnull(Partition2)=0) then
      select concat('insert into temp_sqlth_data_1 select t_stamp,tagid,floatvalue,intvalue from ',Partition2,' where t_stamp>=',start_tstamp-300000,' and t_stamp<=',stop_tstamp,' and tagid in (',HoleDepthTagID,',',ToolfaceTagID,',',QuillOscEnabledTagID,',',BitDepthTagID,')') into @s;
      prepare stmt from @s; execute stmt; deallocate prepare stmt;
   end if;
  ...
  /* Prime a table with 1 second increments for the requested time range */
   ...
   while bigintCounter<=stop_tstamp do
      ...
      update temp_RigState_results set QuillOscEnabled=(select ifnull(floatvalue,intvalue) from temp_sqlth_data_3 where t_stamp<bigIntCounter order by t_stamp desc limit 1) where onesecond_epoch=bigIntCounter and QuillOscEnabled is null; commit; commit;
