...
BEGIN
   ...
   select concat('create temporary table temp_sqlth_data_1 select t_stamp,tagid,floatvalue,intvalue from ',Partition1,' where t_stamp>=',start_tstamp,' and t_stamp<=',stop_tstamp,' and tagid in (',HoleDepthTagID,',',BitDepthTagID,',',SP_TagID,',',MP1_TagID,',',MP2_TagID,',',HookloadTagID,',',BlockHeightTagID,',',TorqueTagID,',',WeightOnBitTagID,',',ROP_TagID,',',RPM_TagID,')') into @s;
   ...
   select rigStateDateRangeData.tenseconds_epoch,StandpipePressure.tagvalue as StandpipePressure,HoleDepth.tagvalue as HoleDepth,BitDepth.tagvalue as BitDepth,hookload.tagvalue as Hookload
	 ...
	 from rigStateDateRangeData
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_1 data group by round(data.t_stamp,-4)) StandpipePressure on rigStateDateRangeData.tenseconds_epoch=StandpipePressure.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_2 data group by round(data.t_stamp,-4)) HoleDepth on rigStateDateRangeData.tenseconds_epoch=HoleDepth.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_3 data group by round(data.t_stamp,-4)) BitDepth on rigStateDateRangeData.tenseconds_epoch=BitDepth.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_4 data group by round(data.t_stamp,-4)) hookload on rigStateDateRangeData.tenseconds_epoch=hookload.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_5 data group by round(data.t_stamp,-4)) mp1 on rigStateDateRangeData.tenseconds_epoch=mp1.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_6 data group by round(data.t_stamp,-4)) mp2 on rigStateDateRangeData.tenseconds_epoch=mp2.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_7 data group by round(data.t_stamp,-4)) BlockHeight on rigStateDateRangeData.tenseconds_epoch=BlockHeight.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_8 data group by round(data.t_stamp,-4)) Torque on rigStateDateRangeData.tenseconds_epoch=Torque.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_9 data group by round(data.t_stamp,-4)) wob on rigStateDateRangeData.tenseconds_epoch=wob.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_10 data group by round(data.t_stamp,-4)) rop on rigStateDateRangeData.tenseconds_epoch=rop.nearest10seconds_epoch
          left outer join (select round(data.t_stamp,-4) as nearest10seconds_epoch,max(ifnull(data.floatvalue,data.intvalue)) as tagvalue from temp_sqlth_data_11 data group by round(data.t_stamp,-4)) rpm on rigStateDateRangeData.tenseconds_epoch=rpm.nearest10seconds_epoch
   ;
